        ...
        (function(){ function _stamp(){ var b=document.getElementById('ver-badge'); if(b) b.textContent='\uD83C\uDFA8 \uC2DC\uC548 \uB9DE\uCDA4 \u00B7 v0806-4 \u00B7 JS\u2713'; }
        ...
        function authSuccess(r) {
            ...
            setTimeout(function(){ ov.style.display = 'none'; ov.classList.remove('cgv-unlocking'); }, 270);  // 로그인 조용히 걷힘
            setTimeout(function(){ _revEls.forEach(function(el){ if (el) el.classList.remove('cgv-reveal'); }); }, 580);
...
function showKakaoModal(text, forced) {
            ...
            for (var ti = 0; ti < filtered.length; ti++) { var t = filtered[ti];
                ...
                if (isR) {
                    var rejCardHtml = "<div class='bg-orange-50 rounded-[28px] p-5 border-2 border-orange-300 mb-4 card-shadow'>"
                        ...
                        + "<div class='text-[11px] text-orange-800 font-bold bg-orange-100 px-3 py-2 rounded-xl mt-2'>\uAC70\uC808\uB41C \uC9C0\uC6D0\uC790: "+getGenderEmoji(t.subName)+" "+t.subName+(_inPos?" ["+_inPos+"]":"")+"</div>"
                ...
                if (isExp && !isD) {
                    ...
                    var cardHtml = "<details class='mb-4 rounded-[24px] border-2 border-slate-200 bg-slate-50 overflow-hidden opacity-70'>"
                        ...
                        + (!isU ? "<div class='text-[11px] text-slate-500 mt-2'>\uC9C0\uC6D0\uC790: "+t.subName+" "+(_inPos?"["+_inPos+"]":"")+"</div>" : "")
                        + "</div></div></details>";
        ...
        function submitFormRequest() {
            ...
            var sentName = _formSelectedName; // clearFormReqName() 호출 전에 미리 저장
            var sentType = _formCurrentType;
        ...
        function buildAbsentForm(name, today) {
            ...
            return DOC_STYLE
                ...
                + '</tbody></table>'
                + '</div>'
                + '<p class="sub-date" style="margin:14px 0 12px">위와 같이 사유서를 제출합니다.</p>'
        ...
        function buildPrivacyForm(name, today) {
            return DOC_STYLE
                ...
                + '<div style="font-size:20px;font-weight:900;text-align:center;letter-spacing:0.04em;margin-bottom:18px;margin-top:4px;line-height:1.5">개인정보 취급자에 대한<br>개인정보보호 서약서</div>'
                ...
                + '</div>'
                + '<p style="text-align:center;font-size:13px;font-weight:700;color:#222;margin:18px 0 22px">'
                + '날&nbsp;짜&nbsp;: <input id="ff-date" class="di" type="date" value="' + today + '" style="font-size:13px;font-weight:700;color:#111;border-bottom:1.5px solid #333">'
                + '</p>'
        ...
        function buildOvertimeForm(name, today) {
            return DOC_STYLE
                + '<div style="font-size:22px;font-weight:900;text-align:center;letter-spacing:0.06em;margin-bottom:22px;margin-top:8px">연장·야간·휴일 근로 동의서</div>'
                + '<table class="dt" style="margin-bottom:22px"><tbody>'
                + '<tr><th style="width:32%">◎ 성&nbsp;&nbsp;&nbsp;명</th><td class="fc"><input id="ff-name" class="di" value="' + name + '" placeholder="이름" style="font-weight:700;font-size:14px"></td></tr>'
                + '<tr><th>◎ 소&nbsp;&nbsp;&nbsp;속</th><td class="ac" style="font-weight:700">CGV동두천</td></tr>'
                + '<tr><th>◎ 주민등록번호</th><td class="fc"><input id="ff-birth" class="di" type="tel" inputmode="numeric" placeholder="앞자리 6자리" maxlength="6" style="font-size:14px"></td></tr>'
                + '</tbody></table>'
                + '<div style="font-size:12px;font-weight:600;color:#222;line-height:2.0;word-break:keep-all;margin-bottom:26px;padding:0 2px">'
                ...
                + '</div>'
                + '<p style="text-align:center;font-size:13px;font-weight:700;color:#222;margin:20px 0">'
                + '날&nbsp;짜&nbsp;: <input id="ff-date" class="di" type="date" value="' + today + '" style="font-size:13px;font-weight:700;color:#111;border-bottom:1.5px solid #333">'
                + '</p>'
        ...
        function buildWorkConditionForm(name, today) {
            return DOC_STYLE
                ...
                + '<td class="fc"><input id="ff-name" class="di" value="' + name + '" style="font-weight:700;font-size:14px"></td>'
                + '<th style="background:#f0f0f0">생년월일</th>'
                + '<td class="fc"><input id="ff-birth" class="di" type="tel" inputmode="numeric" placeholder="예) 001215" maxlength="6" style="font-size:14px"></td>'
                + '</tr>'
                + '<tr>'
                ...
                + '</div>'
                + '<p style="text-align:center;font-size:13px;font-weight:700;color:#222;margin:18px 0">'
                + '날&nbsp;짜&nbsp;: <input id="ff-date" class="di" type="date" value="' + today + '" style="font-size:13px;font-weight:700;color:#111;border-bottom:1.5px solid #333">'
                + '</p>'
        ...
        function buildViewDoc(type, fd, sub, adminName, adminSigBase64) {
            ...
            if (type === 'absent') {
                ...
                return DOC_STYLE
                    ...
                    + '</tbody></table>'
                    + '</div>'
                    + '<p class="sub-date" style="margin:14px 0 10px">위와 같이 사유서를 제출합니다. &nbsp;&nbsp; 제출일: ' + today + '</p>'
                    + '<div style="display:flex;flex-direction:column;gap:12px;margin-top:8px">'
                    + '<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">'
                    + '<span style="font-size:11px;font-weight:800;color:#222;white-space:nowrap">제출자 :</span>'
            ...
            if (type === 'resign') {
                ...
                return DOC_STYLE
                    + '<div style="font-size:32px;font-weight:900;text-align:center;letter-spacing:0.08em;margin-bottom:4px;margin-top:8px">사&nbsp;&nbsp;직&nbsp;&nbsp;원</div>'
                    + '<div style="text-align:center;font-size:13px;color:#555;font-weight:700;margin-bottom:16px">(미소지기 용)</div>'
                    + (fd.resignYMD ? '<p style="font-size:11px;font-weight:700;color:#222;line-height:1.8;margin-bottom:14px;text-align:center">상기 본인은 <b>' + fd.resignYMD + '</b> 자로 당 사를 사직하고자 하오니 재가하여 주시기 바랍니다.</p>' : '')
                    + '<div style="overflow-x:auto;-webkit-overflow-scrolling:touch">'
                    + '<table class="dt" style="min-width:300px"><tbody>'
                    + '<tr><th style="width:22%">성&nbsp;&nbsp;&nbsp;명</th><td class="fc">' + tv(fd.name) + '</td>'
                    + '<th style="width:26%">주민번호 앞자리</th><td class="fc">' + tv(fd.birth) + '</td></tr>'
